...
{
    public class Unit_TesterDarkFog : Unit
    {

        
        public Unit_TesterDarkFog(Location loc,SocialGroup soc) : base(loc,soc)
        {
            maxHp = 5;
            hp = 5;
        }
        ...
        public override void turnTickInner(Map map)
        {
            ...
            if (this.location.soc != null && this.location.soc.hostileTo(this) && (this.location.hex.cloud is Cloud_Fog == false))
            {
                shouldFlee = true;
            }
            if (!shouldFlee)
            ...
            foreach (Location loc in map.locations)
            {
                ...
                if (loc.soc.hostileTo(this) && (loc.hex.cloud is Cloud_Fog == false)) { continue; }
                if (loc.person() != null && loc.soc is Society && loc.person().shadow < 1)
                {
                    ...
                    if (good)
                    {
                        ...
                        //    target = loc;
                        //}
                        if (loc.hex.cloud is Cloud_Fog) { score *= 10; }
